...
:ShapeNetL1 modifications: Konsoulas Dimitrios <dimitrys.konsoulas@gmail.com>
"""
...
#Loads the generated dataset from an npz file
def load_dataset_npz(input_path):
...
flags.DEFINE_bool(
    'use_gpu',
    default=False,
    help='Asks tensorflow to use the GPU instead of just the CPU when training')
...
flags.DEFINE_bool(
    ...
    default=False,
    help='Load dataset from file if set.')

flags.DEFINE_string(
    'loss_type', default="L2",
    help="Loss function type. It can be either 'L2' or 'L1'")
...
flags.DEFINE_float(
    'gamma', default=0.5, help="Trade-off parameter for shape constraint. For L2 recommended 0.5 and for L1 0.0078125")
...
def tikho_lossL1(y_pred, y_true):
    #Same as tikho_loss, but it implements norm L1 instead of L2
    residual = y_true-y_pred
    data_fid = tf.keras.backend.sum(
        tf.keras.backend.abs(residual),axis=(1,2,3))/2. #Removed squaring
    if FLAGS.shape_constraint == 'None':
        shape_constraint = 0
    elif FLAGS.shape_constraint == 'single':
        shape_constraint = single_window_metric(y_pred,y_true)
...
def L1LossMetric(y_pred, y_true):
    residual = y_true-y_pred
    data_fid = tf.keras.backend.sum(
        tf.keras.backend.abs(residual),axis=(1,2,3))/2.
    return data_fid

def L2LossMetric(y_pred, y_true):
    residual = y_true-y_pred
    data_fid = tf.keras.backend.sum(
        tf.keras.backend.square(residual),axis=(1,2,3))/2.
    return data_fid
...
def main(argv):
    if not FLAGS.use_gpu:
        os.environ["CUDA_VISIBLE_DEVICES"] = "" #force use cpu
        print("Using CPU only to train...")
    if FLAGS.loss_type == "L2":
        print("Using L2 norm for loss function")
    elif FLAGS.loss_type == "L1":
        print("Using L1 norm for loss function")
    else:
        print("Wrong loss type! Acceptable values: L2, L1. Exiting...")
        sys.exit()
    if FLAGS.shape_constraint != 'None':
        U = cl.makeUi(FLAGS.n_row,FLAGS.n_col)
        if FLAGS.shape_constraint == 'single':
            print("Training for ShapeNet single-window...")
            U_tensor = tf.reshape(U, [U.shape[0],1,*U.shape[1:],1])
        if FLAGS.shape_constraint == 'multi':
            print("Training for ShapeNet multi-window...")
        ...
        print("Training for Tikhonet...")

    # DATASET LOAD OR GENERATE SECTION
    print("Gamma set to: ", FLAGS.gamma)
    if FLAGS.load_dataset_from_file:
        print("Loading dataset from file saved_dataset.npz")
        dset = load_dataset_npz("saved_dataset.npz")
        dset = dset.shuffle(buffer_size=49000, reshuffle_each_iteration=True)
        dset = dset.repeat()
        dset = dset.map(pre_proc_unet)
        dset = dset.batch(FLAGS.batch_size)
    else:
        print("Generating dataset with galaxy2galaxy internally...")
    ...
    # Compile the model
    # metrics is responsible of the SHOWN metrics during training (just the output)
    metrics = [tf.keras.metrics.mse]#,'accuracy']
    if FLAGS.loss_type == "L2":
        metrics += [L2LossMetric]
    elif FLAGS.loss_type == "L1":
        metrics += [L1LossMetric]
    ...
    if FLAGS.loss_type == "L2":
        ...
        model.compile(optimizer = tf.keras.optimizers.Adam(lr=1e-3)
                  , loss = tikho_lossL1, weighted_metrics=metrics)
    # Train model
    history = model.fit(dset, steps_per_epoch=FLAGS.steps,epochs=FLAGS.epochs)
    
    # Save model
    if FLAGS.loss_type == "L2":
        model_name = 'L2_tikhonet_{0}-constraint_scales-{1}'\
                  .format(FLAGS.shape_constraint,FLAGS.n_scale)
    elif FLAGS.loss_type == "L1":
        model_name = 'L1_tikhonet_{0}-constraint_scales-{1}'\
                  .format(FLAGS.shape_constraint,FLAGS.n_scale)
    if FLAGS.shape_constraint != 'None':
        model_name += '_gamma-{}'.format(FLAGS.gamma)
        if FLAGS.shape_constraint == 'multi':
            model_name += '_shearlet-{}'.format(FLAGS.n_shearlet)
    ...
    #make sure the directory path exists before saving, so we don't fail here
    save_path = os.path.join(FLAGS.model_dir, model_name)
